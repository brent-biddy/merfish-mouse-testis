...
Guidance for Claude Code (claude.ai/code) working in this repository. See `AGENTS.md` for
the invariants — this file is the working reference: setup, the full per-step workflow, and
the reasoning behind it. `README.md` stays a short landing page; this is where a stale step
list or command would actually be noticed.
